...
export default {
  ...
  mounted() {
    ...
    notify.setUpdateCallback((notifications) => {
      this.notifications = notifications.map(notification => {
        ...
        if (existing) {
          // Keep existing drag state if notification exists
          return {
            ...notification,
            dragOffset: existing.dragOffset || 0,
  ...
  methods: {
    ...
    endDrag(notificationId) {
      ...
      const notificationIndex = this.notifications.findIndex(n => n.id === notificationId);
      if (notificationIndex !== -1) {
        // Check if drag distance is enought to close the notification (80px or 25% of screen width)
        const threshold = Math.min(80, window.innerWidth * 0.25);
        if (absoluteDeltaX > threshold) {
          // If the swipe is enought, close
          this.closeNotification(notificationId);
        } else {
          // If not, returnt it back to original position
